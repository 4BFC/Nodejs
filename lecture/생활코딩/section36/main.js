  ...
  if (pathname === '/') { //전반적으로 라우팅 로직이 사용되었다.
    if (queryData.id === undefined) { //url(queryData.id주소)이 정해진 것 없는 상태
      fs.readdir('./data', function (error, filelist) {
        ...
        let list = templateList(filelist); //해당 매개변수로 받아온다. 이는 상위 templateList함수를 참고하면 된다.
        let template = templateHTML(title, list, `<h2>${title}</h2>${description}`, `<a href="/create">create</a>`);//html 구문을 생성하는 함수인데 직접 생성한 함수이기에 상위에 있는 함수를 참고
    ...
    fs.readdir('./data', function (error, filelist) {
      ...
      // let description = 'Hello, Node.js';
      let list = templateList(filelist); //해당 매개변수로 받아온다. 여기서 이해가 안감.
      let template = templateHTML(title, list, `
